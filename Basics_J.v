...
[intros]タクティックは、このようなことを量化子や仮定をゴールから前提条件に引き上げることで実現してくれます。
...
今求められていることは、[n]を何らかの条件に分割し、先に進めそうな形にすることができないかを検討することです。もし[n]が[O]なら、[beq_nat (n + 1) 0]の結果を得ることはできます。もちろん結果は[false]です。しかしもし[n]が何かの[n']を使って[n = S n']と表せると考えても、我々は[n + 1]の値を得ることはできません。ただ、その式が一つの[S]で始まる（始まらないものは[O]にマッチする）ことに着目すると、[beq_nat]の結果を計算して値を求めることができます。その結果[beq_nat (n + 1) 0]は、やはり[false]になるでしょう。
...
(** この練習問題は前の問題の続きで、2進数に関するものである。前の問題で作成された定義や定理をここで用いてもよい。
    ...
    (b) あなたはきっと、逆方向についての証明をしたほうがいいのでは、と考えているでしょう。それは、任意の2進数から始まり、それを自然数にコンバートしてから、また2進数にコンバートし直したものが、元の自然数と一致する、という証明です。しかしながら、これは正しくありません。！！その原因を説明しなさい。


    (c) 2進数を引数として取り、それを一度自然数に変換した後、また2進数に変換したものを返す関数normalizeを「直接的に」定義し、その性質（2進数を一度自然数に変換した後、また2進数に変換したものと同じものをnormalizeが返すこと）を証明しなさい。
*)
